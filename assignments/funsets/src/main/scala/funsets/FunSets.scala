...
trait FunSets extends FunSetsInterface {
   ...
   */
  def singletonSet(elem: Int): FunSet = y => elem == y
   ...
   * that satisfies `p`.
   */
  def exists(s: FunSet, p: Int => Boolean): Boolean = !forall(p, diff(p,s))
  ...
  //  return 'true' on input 'x' if and only if there exists an element 'a' in 's' such that 'f(a) == x'
